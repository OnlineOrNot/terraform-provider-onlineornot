...
A syntax error stops the save operation.
The error identifies the line and column, starting from 1.
Correct the source and try again.
Terraform reports this API error during the apply operation.
The `terraform validate` command does not check JavaScript syntax.

A successful syntax check does not show that the script contains a test or that the test will pass.
The runtime finds tests, resolves imports, and executes assertions.
No tests, an unavailable module, or a failed assertion can cause an execution failure.
